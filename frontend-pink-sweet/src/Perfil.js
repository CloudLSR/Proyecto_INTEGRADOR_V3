...
const Perfil = ({ setPage }) => {
  ...
  // Carga datos reales del backend sin redirigir si falla
  useEffect(() => {
    ...
    fetch('http://localhost:8081/api/usuarios/perfil', {
      ...
      .catch(() => {}); // Si falla, muestra los campos vacíos sin romper nada
  }, []);
  ...
  return (
    <div style={{ backgroundColor: '#ffffff', minHeight: '100vh', paddingBottom: '80px', fontFamily: 'sans-serif' }}>
      ...
      <section style={{ textAlign: 'center', paddingTop: '40px', paddingBottom: '20px' }}>
        ...
        <h1 style={{ color: '#5A3E41', margin: '10px 0 5px 0', fontFamily: 'Poppins-Bold', fontSize: '30px', letterSpacing: '2px' }}>MI PERFIL</h1>
      ...
      <div style={{ maxWidth: '1100px', margin: '0 auto', display: 'flex', gap: '30px', padding: '0 20px', flexWrap: 'wrap', alignItems: 'stretch' }}>
        ...
        <div style={{ width: '280px', flexShrink: 0, backgroundColor: 'white', border: '2px solid #EAAFB8', borderRadius: '20px', padding: '40px 15px', display: 'flex', flexDirection: 'column' }}>
            ...
            {menuTabs.map((tab) => {
              ...
              return (
                <button 
                  ...
                  style={{ width: '100%', display: 'flex', alignItems: 'center', gap: '15px', backgroundColor: isActive ? '#FADADD' : 'transparent', border: 'none', padding: '14px 20px', fontFamily: isActive ? 'Poppins-Bold' : 'Poppins-Medium', fontSize: '14px', color: isActive ? '#C6676D' : '#5A3E41', textAlign: 'left', cursor: 'pointer', borderRadius: '10px' }}
                >
